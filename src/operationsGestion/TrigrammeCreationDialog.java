...
public class TrigrammeCreationDialog extends JDialog {
	...
	if (authentification.droits == AuthentificationDialog.BoBarman) {
		...
		if (majusculeSuivant) {
		    if (i != 0) {
			prenom = prenom.substring(0, i) + (char) (prenom.charAt(i) - 32) + prenom.substring(i + 1);
		    } else {
		...
		Trigramme trigramme = new Trigramme(parent, champTrigramme.getText(), nom, prenom, surnom,
			champCasert.getText(), champCategorie.getSelectedIndex(), Integer.parseInt("0"
				+ champPromo.getText()), "", champPhoto.getText(), (int) (100 * Double.parseDouble("0"
				+ champSolde.getText())), (int) (100 * Double.parseDouble("0" + champSolde.getText())));
