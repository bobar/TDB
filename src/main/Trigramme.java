...
public class Trigramme {
	...
	public Trigramme(MainWindow parent, String tri) throws Exception {
		...
		if (rs.next()) {
			...
			if (rs.next()) {
				throw new TDBException("Trigramme pas unique");
			}
		} else {
	...
	public void creer(int admin) throws Exception {
		...
		ResultSet rs1 = stmt.executeQuery("SELECT id FROM accounts WHERE trigramme='" + trigramme
				+ "'");
		if (rs1.first()) {
			throw new TDBException("Trigramme existant");
		}
	...
	public void modifier(int id, int admin) throws Exception {
		...
		if (rs.first()) {
			...
			Transaction transaction = new Transaction(id, 0,
					"Modification du trigramme (ancien trigramme : " + rs.getString("trigramme")
							+ ")", admin, (int) (date.getTimeInMillis() / 1000),
					parent.banqueBob.id);
			stmt2 = parent.connexion
					.prepareStatement("INSERT INTO transactions (id,price,comment,admin,date,id2) VALUES (?,?,?,?,?,?)");
			stmt2.setInt(1, transaction.id);
	...
	public void debiter(int montant) throws Exception {
		...
		if (montant >= 0 && montant <= 2000) {
			...
			GregorianCalendar date = new GregorianCalendar();
			date.setTime(new Date());
			Transaction transaction = new Transaction(id, -montant, "", 0,
