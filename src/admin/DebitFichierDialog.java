...
public class DebitFichierDialog extends JDialog {
	...
	MainWindow parent;
	DebitFichierDialogListener listener = new DebitFichierDialogListener(this);
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				for (int i = 0; i < modele.getRowCount(); i++) {
					...
					try {
						...
						int montant = (int) (100 * Double.parseDouble((String) modele.getValueAt(i,
								3)));
				...
				if (validation) {
					...
					PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(
							fichier.replace(".csv", "") + "_refuses.csv", true)));
					int refuses = 0;

					for (int i = 0; i < modele.getRowCount(); i++) {

						if (modele.getValueAt(i, 0).equals(Boolean.TRUE)) {
							int montant = (int) (100 * Double.parseDouble((String) modele
									.getValueAt(i, 3)));
							Trigramme trigramme = new Trigramme(parent, (String) modele.getValueAt(
									i, 1));
							...
							Transaction transaction = new Transaction(trigramme.id, -montant, "",
									adminId, (int) (date.getTimeInMillis() / 1000), banqueId);
