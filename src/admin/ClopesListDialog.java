...
public class ClopesListDialog extends JDialog {
      ...
      try {
        if (arg0.getSource().equals(creerButton)) {
          ...
          ClopesModificationDialog dialog =
              new ClopesModificationDialog(parent, (String) listeClopes.getValueAt(ligneChoisie, 0));
          dialog.executer();
          ...
          int confirmation =
              JOptionPane
                  .showConfirmDialog(parent,
                      "Etes-vous sur de vouloir supprimer " + modele.getValueAt(ligneChoisie, 0)
  ...
  public void refresh() throws Exception {
    ...
    for (Clopes clope : clopes) {
      String[] data = {clope.marque(), "" + (double) clope.prix() / 100, "" + clope.quantite()};
      modele.addRow(data);
    }
    listeClopes.setModel(modele);
    listeClopes.repaint();
